...
var token = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..kI2qJfzpsb6BrYpA7cqecCeNcsLvg3FdcKqdySTvh2g"//localStorage.getItem('token');
console.log(token);
var question_title
...
post_answer (data){
  ...
  if(response.status === 201){
    response.json().then(function(data){
      ...
      questionDetails.get_question_details()
      loader.style.display = "none";
    });
  }
  else if (response.status === 400) {
...
upvote_current_answer(event){
  ...
  questionDetails.upvote_answer(id);
}

upvote_answer(id){
  // var id = event.target.answerid
  // loader.style.display = "block"
  var url = url =" http://127.0.0.1:5000/api/v1/upvote/"+id
...
downvote_current_answers(event){
  ...
  id = event.target.answerid;
  questionDetails.downvote_answers(id)
}
...
downvote_answers(id){
  ...
  // loader.style.display = "block"
  // var id = event.target.answerid
  var url = url =" http://127.0.0.1:5000/api/v1/downvote/"+id
  var data = JSON.stringify({})
  return fetch(url,{
    method:"PATCH",
    body:data,
...
mark_as_prefered(id){
  // event.preventDefault();
  // loader.style.display= "block"
  var url = " http://127.0.0.1:5000/api/v1/mark_answer/"+id
  var data = JSON.stringify({})
  return fetch(url,{
    method:"PATCH",
    body:data,
